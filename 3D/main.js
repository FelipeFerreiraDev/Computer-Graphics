function main() {
    ...
    // Cria e posiciona as esferas (átomos) com materiais diferentes
    const materials = [
    ...
    // Cria e posiciona os torus (ligações)
    for (let i = 0; i < 4; i++) {
        const torusGeometry = new THREE.TorusGeometry(torusRadius, torusRadius / 5, 16, 32);
        const torusMaterial = new THREE.MeshPhongMaterial({ color: colors[i] });
        const torus = new THREE.Mesh(torusGeometry, torusMaterial);
        torus.rotation.x = Math.PI / 2;
        torus.position.set(Math.cos((i * Math.PI) / 2) * boxSize, Math.sin((i * Math.PI) / 2) * boxSize, 0);
        reactSymbol.add(torus);
    }

    // Cria um cubo (caixa) para envolver o símbolo com textura
    const cubeTextureLoader = new THREE.TextureLoader();
    const cubeTexture = cubeTextureLoader.load('/textura.png'); // Substitua pelo caminho correto para a textura do cubo
